...
- [x] 50–100 production-quality low-poly assets — **489**, from three CC0 Kenney kits, licence confirmed from the file bundled in each pack rather than from memory. `pnpm import-pack` is the reusable path and **refuses to run without a licence, an author and a source URL**. Models are re-encoded rather than copied, which the first attempt got wrong: two kits reference a sibling texture atlas instead of embedding it, and the two atlases differ under the same filename, so a copy would have left 150 unopenable models and a shared path would have repainted one kit with the other's textures
- [x] 5–10 polished starter templates — **eight**. Castle Siege, Survivor's Camp and Rocky Canyon each show sculpted terrain, varied props, an enemy behaviour and a trigger. A new test checks every placed `assetId` resolves in the generated manifest, which the schema cannot: a string naming no asset is a valid document describing a scene of grey boxes, and that is the worst thing a _starter_ template can be
- **Still open:** animated enemies. No CC0 pack sourced here has them, so that category remains a single placeholder — a static model would not have closed the task
...
- **DoD: not met, because it cannot be run here.** It asks for a usability test with five people unfamiliar with the product, and there are no people in this environment. Everything else the sprint asks for is done, each with checks that keep it done.

**Sprint 38: Docs, onboarding, beta launch**

- [~] Public docs — behaviour reference, export guide, troubleshooting FAQ, **without a Docusaurus site**. The reference is generated by `pnpm gen-docs` from the engine's own Zod schemas and CI fails when the committed page and the code disagree, which is the part that actually decays. A site would add search, versioning and a URL — and a second app to keep alive and a second place docs live, one of which always goes stale. The case is written out in `docs/README.md` rather than left as a silent skip
- [x] In-app onboarding — five steps, all skippable. The flag is written on **skip**, not on completion: a tour you escaped from that greets you again on reload is worse than never showing one, and two tests assert that from both exits. Steps whose element is missing are dropped rather than left ringing empty space
- [ ] **Not done:** 3–5 video walkthroughs. Needs a recorder, a voice and a person
- [ ] **Not done:** a closed beta cohort of 20–50 users. Needs people
- [x] Funnel tracking, signup through export. A closed event vocabulary in the schema, properties narrowed to counts and durations — no project names, no asset ids, no scene contents, enforced by a `.strict()` schema and asserted by a test. **`PostHogSink` has never sent a request**, because no account exists; `LocalAnalytics` runs and the drop-off table is inspectable through the dev API
...
- [ ] **Not done:** fix the top friction points. The stand-in for beta data was the export visual regression red in CI, and **it is still red on all five templates**. Two hypotheses were wrong — the reference grid (hiding it moved Empty field from 39,710 differing pixels to 40,750, which is nothing) and trigger wireframes. Instancing, shadow-map type and antialiasing all match between editor and export. The suite did gain something worth keeping: `hideEditorFurniture` now _asserts_ it hid something, because the first version hid nothing, returned success, and got reported as a fix on the strength of a grep too narrow to contradict it
