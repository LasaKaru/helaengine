...
- [x] Commission or model 50-100 production-quality low-poly assets — **489 models**, from Kenney's Nature, Survival and Castle kits, all CC0 and confirmed from the licence file bundled in each pack. Trees 133, rocks 128, props 101, buildings 88, terrain 38. Every one carries `license`, `author`, `sourceUrl` and `origin: 'third-party'`, because `pnpm import-pack` refuses to run without them. **Still missing: animated enemies** — these kits have none, so that category is one first-party placeholder and a static model would not have closed it
- [x] Build 5-10 polished starter templates — **eight**, with Castle Siege, Survivor's Camp and Rocky Canyon added. Each has sculpted terrain, varied props, an enemy with a behaviour and a trigger volume. They exist now and not before for a concrete reason: a curtain wall needs straights _and_ corners _and_ a gate, and a canyon needs cliff blocks laid as a corridor. A new test asserts every placed `assetId` resolves in the generated manifest, which the schema check cannot do — a string naming no asset is a valid document describing a scene of grey boxes
...
- [x] Full accessibility/UX pass over the core editor flows — written as **five tests that run in CI**, not a one-off review. Found a _critical_ `aria-required-children` (react-window wraps cells in `role="row"`; ours were plain divs, so the asset library was a grid with no cells) and a contrast failure at 3.73:1 on the category counts
...
**Deliverables:** Rich asset library and template set — **delivered**, with one gap: no animated enemies, because no CC0 pack sourced here has them.

**Definition of Done: not met, because it cannot be run here.** It asks for a usability test with five people unfamiliar with the product, and there are no people in this environment. Everything the sprint could produce without them is done, each with automated checks that keep it done: the library, the templates, attribution, accessibility, and the first-run states. The usability test is the one thing left, and it needs users rather than another commit.
...
- [ ] **NOT DONE.** Fix the top-ranked friction points. No beta data, so the substitute was the export visual regression that has been red in CI — and **it is still red**. Two hypotheses were wrong: hiding the editor's reference grid changed the Empty field diff from 39,710 pixels to 40,750, i.e. nothing, and trigger wireframes did not account for the rest either. Instancing thresholds, shadow-map type and antialiasing all match between editor and export. What the suite _did_ gain is an assertion that the furniture-hiding actually hid something — added because the first attempt silently hid nothing and I reported it as fixed on the strength of a grep too narrow to show otherwise
