...
export class ChaseOnSightBehavior implements Behavior {
  ...
  onUpdate(_object: GameObject, deltaSeconds: number): void {
  ...
  onEvent(object: GameObject, event: string, payload: unknown): void {
    ...
    if (context.health <= DEAD) machine.changeTo('dead');
    else if (machine.current === 'patrol' || machine.current === 'idle') machine.changeTo('chase');
  }
